<x-app-layout>
    <x-slot name="header">
        <x-ui.section-header title="Editar recurrente" eyebrow="Ajusta la periodicidad o el importe.">
            <x-slot:action>
                ...
                <form method="POST" action="{{ route('recurring.update', $recurring) }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <x-ui.select id="type" name="type" label="Tipo" required>
                            <option value="expense" {{ old('type', $recurring->type) === 'expense' ? 'selected' : '' }}>Gasto</option>
                            <option value="income"  {{ old('type', $recurring->type) === 'income' ? 'selected' : '' }}>Ingreso</option>
                        </x-ui.select>
                        <x-ui.input id="amount" name="amount" type="number" label="Importe" required
                                    step="0.01" min="0.01" :value="old('amount', $recurring->amount)" />
                    </div>

                    <x-ui.select id="category_id" name="category_id" label="Categoría" required>
                        <option value="">Selecciona…</option>
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" {{ (string) old('category_id', $recurring->category_id) === (string) $cat->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <x-ui.select id="frequency" name="frequency" label="Frecuencia" required>
                            @foreach (\App\Models\RecurringTransaction::FREQUENCIES as $slug => $label)
                                <option value="{{ $slug }}" {{ old('frequency', $recurring->frequency) === $slug ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </x-ui.select>
                        <x-ui.input id="interval" name="interval" type="number" label="Cada (N)" required
                                    min="1" max="60" :value="old('interval', $recurring->interval)" />
                    </div>
