...
@section('content')
    <div class="mb-6">
        <x-ui.section-header title="Nuevo usuario" eyebrow="Crea una cuenta y asigna un rol." :back="route('admin.users.index')" />
    </div>
        ...
        <x-ui.card padding="p-7">
            <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-5">
                ...
                <x-ui.input id="name" name="name" type="text" label="Nombre"
                            ...
                            :error="$errors->first('name')" placeholder="Nombre completo" />

                <x-ui.input id="email" name="email" type="email" label="Correo electrónico"
                            :value="old('email')" required
                            :error="$errors->first('email')" placeholder="usuario@correo.com" />

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-ui.input id="password" name="password" type="password" label="Contraseña"
                                required :error="$errors->first('password')" placeholder="Mínimo 8 caracteres" />

                    <x-ui.input id="password_confirmation" name="password_confirmation" type="password"
                                label="Confirmar contraseña" required placeholder="Repite la contraseña" />
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative flex items-center gap-2 p-3.5 border-2 rounded-xl cursor-pointer transition focus-within:ring-2 focus-within:ring-brand-500/30
                            {{ old('role', 'user') === 'user' ? 'border-brand-500 bg-brand-50/50 dark:bg-brand-950/30' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 bg-white dark:bg-slate-900' }}">
                        ...
                        <label class="relative flex items-center gap-2 p-3.5 border-2 rounded-xl cursor-pointer transition focus-within:ring-2 focus-within:ring-brand-500/30
                            {{ old('role') === 'admin' ? 'border-brand-500 bg-brand-50/50 dark:bg-brand-950/30' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 bg-white dark:bg-slate-900' }}">
